  ...
  return (
    <article className="rounded-2xl border border-[var(--accent-100)] bg-[var(--card-bg)] p-4 shadow-md">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm">
        ...
        <Link
          className="rounded-lg border border-[var(--accent-200)] px-3 py-2 text-center font-semibold text-[var(--accent-900)]"
          to={`/my-menus/${menu.id}/foods?menuId=${menu.id}`}
          state={{ menuName: menu.name }}
          onClick={() => localStorage.setItem('selected-menu-id', menu.id)}
        >
          Gerenciar pratos
        </Link>
      </div>
